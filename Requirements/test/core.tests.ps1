...
Describe "Core" {
  Context "applyRequirement" {
    ...
    It "Should output all log events" {
      ...
      $expectedIds = "Test","Set","Validate" | ForEach-Object { "$_-Start","$_-Stop" }
      $foundIds = $events | ForEach-Object { "$($_.Method)-$($_.State)" }
      $expectedIds | ForEach-Object { $_ -in $foundIds | Should -BeTrue }
    }
    It "Should provide the result of the 'Test'" {
      ...
      $event | Select-Object -First 1 -Skip 1 | ForEach-Object Result | Should -Be $expected
    }
  ...
  Context "applyRequirements" {
    It "Should call 'Test' on each requirement" {
      ...
      $requirements = 1..3 | ForEach-Object {
        @{
          Describe = "Simple Requirement"
